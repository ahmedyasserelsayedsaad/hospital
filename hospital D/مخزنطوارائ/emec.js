...
//creat function
let carr;
if(localStorage.productemec!=null){
    carr=JSON.parse(localStorage.productemec)
}else{
    carr=[];
}
creat.onclick=()=>{
...
if(mood==='create'){
    ...
    localStorage.setItem('productemec',JSON.stringify(carr));
...
function dindex(i){
 carr.splice(i,1);
 localStorage.productemec=JSON.stringify(carr);
 showdata();
}
